...
public class Encrypter1 {
    ...
    private static final byte[] staticSalt = "{Peace, Love and Unity!}@allPlaces4Ever".getBytes(charset);

    public static String encrypt(String password) {
	byte[] randomSalt = EncryptionUtilities.generateRandomBytes(DEFAULT_SALT_LENGTH);
	...
	initializeAndSaltPasswordHash(randomSalt, staticSalt, password, passwordHash);
    ...
    private static byte[] iterate(byte[] randomSalt, byte[] staticSalt, byte[] passwordHash) {
	try {
		...
		byte[] saltedPasswordHash = new byte[passwordHashLen + staticSaltLen + randomSaltLen];
	    ...
	    throw new RuntimeException("Expected SHA-512 algorithm is not provided!", e);
	}
    }

    private static void initializeAndSaltPasswordHash(byte[] randomSalt, byte[] staticSalt, byte[] password,
	    byte[] passwordHash) {
	int randomSaltLen = randomSalt.length;
	int staticSaltLen = staticSalt.length;
