...
public class PasswordStrengthCalculator {
	...
	try {
		...
		throw new RuntimeException(
			"Could not find '" + PASSWORD_CHECK_PROPERTY_FILE + "' for password strength settings");
	    }
	    try {
		properties.load(inputStream);
		PASSWORD_MIXED_CASE = Integer.valueOf(properties.getProperty("password.mixed.case"));
		PASSWORD_MAX_LENGTH = Integer.valueOf(properties.getProperty("password.max.length"));
		PASSWORD_MIN_LENGTH = Integer.valueOf(properties.getProperty("password.min.length"));
		PASSWORD_NUMERIC = Integer.valueOf(properties.getProperty("password.numeric"));
		PASSWORD_SPECIAL = Integer.valueOf(properties.getProperty("password.special"));
		PASSWORD_STRENGTH = Integer.valueOf(properties.getProperty("password.strength"));
	    } finally {
		inputStream.close();
	    }
	} catch (IOException e) {
	    throw new RuntimeException("Could not read properties for PasswordCheck! (PasswordCheck.properties)", e);
	}
    }
